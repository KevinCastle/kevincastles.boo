...
const Tooltip = ({ message, children, top=false }) => {
    return (
        <div className="group/tooltip relative flex">
            {children}
            <span className={`absolute hidden lg:block  ${ top ? 'lg:top-[-60%] 2xl:top-[-50%]' : 'lg:bottom-[-60%] 2xl:bottom-[-50%]'} left-[50%] translate-x-[-50%] scale-0 transition-all rounded bg-secondary-700 p-2 text-xs text-light group-hover/tooltip:scale-100`}>
